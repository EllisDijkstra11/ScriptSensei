...
document.addEventListener('DOMContentLoaded', function () {
  ...
  //Troep
  {
  // Assuming intersection is your array of points with { point, speed, curvature }
  function drawDots(points, color, size) {
    // Assuming 'canvas' is your SVG container element (an <svg> element)
    const canvas = document.getElementById('canvas');

    // Loop through each point in intersection
    points.forEach(item => {
      const point = item.point; // Assuming point is an array [x, y]

      // Create a <circle> element
      const circle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
      
      // Set circle attributes
      circle.setAttribute('cx', point[0]); // x-coordinate of the center
      circle.setAttribute('cy', point[1]); // y-coordinate of the center
      circle.setAttribute('r', size); // Radius of the circle
      circle.setAttribute('fill', color); // Color of the circle
      
      // Optionally, you can set other attributes like stroke, stroke-width, etc.
  ...
  function displaySimplifiedPath(simplifiedPath) {
    // Create a new SVG element (a path)
    const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    const d = simplifiedPath.map((point, index) => (index === 0 ? 'M' : 'L') + point.join(',')).join(' ');    
    path.setAttribute('d', d);
    path.setAttribute('stroke', 'black'); // Use a different color for simplified paths
    path.setAttribute('stroke-width', strokeWidth);
    path.setAttribute('fill', 'none');
